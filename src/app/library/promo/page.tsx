...
export default function PromoPage() {
  ...
  const handleImport = (mapping: Record<string, string>, rows: string[][]) => {
    ...
    const imported: Promo[] = rows.map((row) => {
    ...
    setPromos((prev) => [...prev, ...imported]);
  };

  const onExport = () => {
    handleExport(
      "csv",
      ["Name", "Type", "Value", "Start Date", "End Date", "Status"],
      promos.map((p) => [
        p.name,
        p.type,
        String(p.value),
    ...
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <h1 className="text-lg font-semibold text-gray-800">Promo</h1>
          <span className="text-sm text-gray-500">{promos.length} total</span>
        </div>
      ...
      {/* Table */}
      {filtered.length === 0 ? (
        <EmptyState title="No Data To Display" description="No promos found." />
      ) : (
        <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((promo) => (
                <tr key={promo.id} className="border-t border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <button onClick={() => openEdit(promo)} className="text-blue-500 text-xs hover:underline">Edit</button>
                    <span className="mx-1 text-gray-300">|</span>
                    <button onClick={() => handleDelete(promo.id)} className="text-red-500 text-xs hover:underline">Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <ImportModal
        ...
        fields={["name", "type", "value", "startDate", "endDate"]}
