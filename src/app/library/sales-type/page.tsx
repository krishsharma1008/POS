...
import { initialSalesTypes, type SalesType } from "@/lib/mock-data/library";
import ImportModal from "@/components/ui/ImportModal";
import EmptyState from "@/components/ui/EmptyState";
import { handleExport } from "@/lib/export";

export default function SalesTypePage() {
  const [salesTypes, setSalesTypes] = useState<SalesType[]>(initialSalesTypes);
  ...
  const handleSave = () => {
    if (editing) {
      setSalesTypes((prev) =>
        prev.map((s) =>
          s.id === editing.id
            ? { ...s, name: formName, active: formActive }
            : s
        )
      );
    } else {
  ...
  const handleImport = (mapping: Record<string, string>, rows: string[][]) => {
    ...
    setSalesTypes((prev) => [...prev, ...imported]);
  };
  ...
  const onExport = () => {
    handleExport(
      "csv",
      ["Name", "Status"],
      salesTypes.map((s) => [s.name, s.active ? "Active" : "Inactive"]),
      "sales-types"
    );
  };
    ...
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <h1 className="text-lg font-semibold text-gray-800">Sales Type</h1>
          <span className="text-sm text-gray-500">{salesTypes.length} total</span>
        </div>
      ...
      {/* Table */}
      {filtered.length === 0 ? (
        <EmptyState title="No Data To Display" description="No sales types found." />
      ) : (
            ...
            <tbody>
              {filtered.map((s) => (
                <tr key={s.id} className="border-t border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <button onClick={() => openEdit(s)} className="text-blue-500 text-xs hover:underline">Edit</button>
          ...
          <div className="bg-white rounded-lg shadow-xl w-[480px]">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
              <h2 className="text-lg font-semibold text-gray-800">
                {editing ? "Edit Sales Type" : "Create Sales Type"}
              </h2>
