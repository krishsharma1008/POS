...
import { initialGratuities, type Gratuity } from "@/lib/mock-data/library";
import ImportModal from "@/components/ui/ImportModal";
import EmptyState from "@/components/ui/EmptyState";
import { handleExport } from "@/lib/export";

export default function GratuityPage() {
  const [gratuities, setGratuities] = useState<Gratuity[]>(initialGratuities);
  const [search, setSearch] = useState("");
  const [showModal, setShowModal] = useState(false);
  const [showImport, setShowImport] = useState(false);
  const [editing, setEditing] = useState<Gratuity | null>(null);
  ...
  const handleSave = () => {
    if (editing) {
      setGratuities((prev) =>
        prev.map((g) =>
          g.id === editing.id
            ? { ...g, name: formName, rate: formRate, active: formActive }
            : g
        )
      );
    } else {
  ...
  const handleImport = (mapping: Record<string, string>, rows: string[][]) => {
    ...
    setGratuities((prev) => [...prev, ...imported]);
  };
  ...
  const onExport = () => {
    handleExport(
      "csv",
      ["Name", "Rate (%)", "Status"],
      gratuities.map((g) => [g.name, `${g.rate}%`, g.active ? "Active" : "Inactive"]),
      "gratuity"
    );
  };
    ...
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <h1 className="text-lg font-semibold text-gray-800">Gratuity</h1>
          <span className="text-sm text-gray-500">{gratuities.length} total</span>
        </div>
      ...
      {/* Table */}
      {filtered.length === 0 ? (
        <EmptyState title="No Data To Display" description="No gratuities found." />
      ) : (
            ...
            <tbody>
              {filtered.map((g) => (
                <tr key={g.id} className="border-t border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">{g.rate}%</td>
                  ...
                  <td className="px-4 py-3">
                    <button onClick={() => openEdit(g)} className="text-blue-500 text-xs hover:underline">Edit</button>
                    <span className="mx-1 text-gray-300">|</span>
                    <button onClick={() => handleDelete(g.id)} className="text-red-500 text-xs hover:underline">Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          <div className="bg-white rounded-lg shadow-xl w-[480px]">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
              <h2 className="text-lg font-semibold text-gray-800">
                {editing ? "Edit Gratuity" : "Create Gratuity"}
              </h2>
