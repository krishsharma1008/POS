...
import { initialTaxes, type Tax } from "@/lib/mock-data/library";
...
export default function TaxesPage() {
  ...
  const handleSave = () => {
    if (editing) {
      setTaxes((prev) =>
        prev.map((t) =>
          t.id === editing.id
            ? { ...t, name: formName, rate: formRate, active: formActive }
            : t
        )
      );
    } else {
  ...
  const handleImport = (mapping: Record<string, string>, rows: string[][]) => {
    ...
    const imported: Tax[] = rows.map((row) => {
  ...
  const onExport = () => {
    handleExport(
      "csv",
      ["Name", "Rate (%)", "Status"],
      taxes.map((t) => [t.name, `${t.rate}%`, t.active ? "Active" : "Inactive"]),
      "taxes"
    );
  };
    ...
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <h1 className="text-lg font-semibold text-gray-800">Taxes</h1>
          <span className="text-sm text-gray-500">{taxes.length} total</span>
        </div>
      ...
      {/* Table */}
      {filtered.length === 0 ? (
        <EmptyState title="No Data To Display" description="No taxes found." />
      ) : (
            ...
            <tbody>
              {filtered.map((t) => (
                <tr key={t.id} className="border-t border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <button onClick={() => openEdit(t)} className="text-blue-500 text-xs hover:underline">Edit</button>
